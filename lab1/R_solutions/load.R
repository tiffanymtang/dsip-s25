...
load_dates_data <- function(path = here::here("data")) {
  ...
  epoch_data <- dplyr::bind_cols(epoch_nums, epoch_dates, epoch_days) |>
    tibble::as_tibble()
  return(epoch_data)
}
...
load_mote_location_data <- function(path = here::here("data")) {
  mote_data <- data.table::fread(
    file.path(path, "mote-location-data.txt")
  ) |>
    setNames(c("ID", "Height", "Direc", "Dist", "Tree")) |>
    tibble::as_tibble()
  return(mote_data)
}
